...
dif_score_file = pd.read_csv('Differential_V8_with_pval_2023.csv')
'''------------------------------------------------------------------------------------------------------------------'''

'''Script calculates percent of genes expressed above FC = 1, i.e. upregulated. To obtain down regulated change condition to -1 '''
condition = 1
...
def percent_FC_bigger_than_1():
        ...
        for item in line:
            if item not in ['ENSG', 'Name']:
                if line['ENSG'] in aut_dict:
                    ...
                    if float(line[item].split(',')[0]) >= condition and float(line[item].split(',')[1]) <= 0.05:
    ...
    writer = csv.DictWriter(open('FC ABOVE 1 per tissue in AUT UPS Chap and SYN con pval.csv', 'w', newline=''), fieldnames=fd)
    writer.writeheader()
    write_row = {}
    ...
    for tissue in fc_dict:
        ...
        for system in fc_dict[tissue]:
            ...
            write_row['Percent FC above 1'] = (fc_dict[tissue][system][0]/fc_dict[tissue][system][1])*100
